...
class hashTableCH
{
    private:
        ...
        int *primeNumbers; // Array of prime numbers that are larger than the original table size
        int maxPossibleTableSize; // Holds the max table size value (The last value in primeNumbers)
    public:
        hashTableCH(int bsize);  // Parameterized constructor to create hash table with given size 
        void createListOfPrimes(int* arr); // Function that creates an array of prime numbers (Populates data member "primeNumbers[50]") 
        ...
        bool rehashHelper(int* newTable1, int* newTable2, int key, int originalKey, int counter); // Modified insert function to insert values into the new hash table
